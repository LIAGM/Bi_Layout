...
def pano_stretch(pano_img, corners, kx, ky, kz, new_label_corners=None):
    """
    :param pano_img: a panorama image, shape must be [h,w,c]
    :param corners:
    :param kx: stretching along left-right direction
    ...
    if new_label_corners is not None:
        new_new_corners = pano_stretch_conner(new_label_corners, kx, ky, kz)
        return new_img, new_corners, new_new_corners
...
class PanoDataAugmentation:
    ...
    def execute_space_aug(self, corners, image, new_corners=None):
        if image is None:
            return image
        ...
        if self.aug is None:
            if new_corners is None:
                ...
                return corners, image, new_corners
        w = image.shape[1]
        h = image.shape[0]
        ...
        if self.need_aug('STRETCH'):
            ...
            if new_corners is None:
                ...
                image, corners, new_corners = pano_stretch(image, corners, kx, ky, kz, new_corners)
        ...
        if self.need_aug('ROTATE'):
            ...
            if new_corners is not None:
                new_corners[..., 0] = (new_corners[..., 0] + pixel2uv(np.array([d_pu]), w, h)) % pixel2uv(np.array([w]), w, h)
        ...
        if self.need_aug('FLIP') and np.random.randint(2) == 0:
            ...
            if new_corners is not None:
                new_corners[..., 0] = pixel2uv(np.array([w]), w, h) - new_corners[..., 0]
    ...
    def execute_aug(self, corners, image, new_corners=None):
        if new_corners is None:
            ...
            corners, image, new_corners = self.execute_space_aug(corners, image, new_corners)
            if image is not None:
                image = self.execute_visual_aug(image)
            return corners, image, new_corners
