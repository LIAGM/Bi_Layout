...
ZIND_BAD_PREDICT_PATH = '/media/Pluto/frank/layout_ambiguity/zind/zind_simple'
# ZIND_OFFICIAL_RESULT_DIR = "/media/Pluto/frank/room_layout_project/checkpoints/SWG_Transformer_LGT_Net/zind_official/results/test_best"
# ZIND_OFFICIAL_RESULT_DIR = "/media/Pluto/frank/room_layout_project/checkpoints/SWG_Transformer_LGT_Net/test_zind_raw_all_brandy_200/results/test_best/"
...
def read_new_label(label_path):

    # corners are uv format 
    new_label = np.loadtxt(label_path)

    output = {
        'new_corners': new_label,
        'id': os.path.basename(label_path).split('.')[0]
    }
    return output
...
def read_zind(partition_path, simplicity_path, data_dir, mode, is_simple=True,
              layout_type='layout_raw', dataset_type='occlusion', is_ceiling_flat=False, plan_y=1):
    ...
    for house_index in house_list:
        ...
        for pano in panos:
            if dataset_type == 'occlusion':
                if 'layout_raw' not in pano or 'layout_visible' not in pano:    # or the idx in __getitem__ will be wrong
                    continue
            ...
            # LGT bad prediction subset
            with open(os.path.join(ZIND_BAD_PREDICT_PATH, f"bad_predictions.txt"), 'r') as f:
