...
_C = CN()
_C.DEBUG = False    # set to True for breakpoint debugging
_C.MODE = 'train'
_C.VAL_NAME = 'val'
_C.TAG = 'default'
...
_C.MODEL.TYPE = 'origin'    # TODO: rename
...
# New Depth loss
_C.TRAIN.CRITERION.NEW_DEPTH = CN()
_C.TRAIN.CRITERION.NEW_DEPTH.NAME = 'new_depth'
_C.TRAIN.CRITERION.NEW_DEPTH.LOSS = 'L1Loss'
_C.TRAIN.CRITERION.NEW_DEPTH.WEIGHT = 0.0
_C.TRAIN.CRITERION.NEW_DEPTH.WEIGHTS = []
_C.TRAIN.CRITERION.NEW_DEPTH.NEED_ALL = False
...
# New Grad(Normal) loss
_C.TRAIN.CRITERION.NEW_GRAD = CN()
_C.TRAIN.CRITERION.NEW_GRAD.NAME = 'new_grad'
_C.TRAIN.CRITERION.NEW_GRAD.LOSS = 'GradLoss'
_C.TRAIN.CRITERION.NEW_GRAD.WEIGHT = 0.0
_C.TRAIN.CRITERION.NEW_GRAD.WEIGHTS = [1.0, 1.0]
_C.TRAIN.CRITERION.NEW_GRAD.NEED_ALL = True
# Object loss
...
# ZInD data subset
_C.DATA.TYPE = 'both' # raw, visible, both -> both will be for two-head
_C.DATA.SIMPLICITY = 'both'  # simple, complex, both
_C.DATA.PRIMARY = 'both'  # primary, secondary, both
...
_C.EVAL.CKPT_OPTION = 'best'

# Evaluation GT mismatch
# In ZInD dataset, raw label is used by baseline methods for evaluation
# However, in our method, raw label is treated as the second label (enclosed type), i.e., the model output is in extended/enclosed order
# When we do all the evaluation, we will use the first label as GT (follow the case in MP3D dataset)
# Therefore, for the ZInD evaluation, we need to exchange the GT definition
# Note1: the evaluation includes model saving in val, diambiguate metric in testing, etc
# Note2: for saving extended/enclosed img, we'll need to exchange back to follow the original order (see main.py for details)
_C.EVAL.EVAL_GT_MISMATCH = False

# Ambiguity detection (Opening detection)
_C.EVAL.OPENING = False
